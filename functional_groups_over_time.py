...
for key, filtered_AS_data_midas in midas_data.items():
    ...
    functional_columns = ["Chemoautotroph/mixotroph", "AOB", "NOB", "Anammox", "Aerobic heterotroph", "PAO", "GAO", "Nitrite_reduction", "Sulfate_reduction", "Fermentation", "Acetogen", "Methanogen"]
    for i in functional_columns:
        filtered_AS_data_midas[i] = filtered_AS_data_midas[i].apply(lambda x: x[0] if len(x) > 0 else None)
    ...
    filtered_AS_data_midas = np.vstack((cols, filtered_AS_data_midas))
    np.save(f'{file_path}{key}_midas_genus_per_week',filtered_AS_data_midas)
    
    filtered_AS_data_midas = pd.DataFrame(filtered_AS_data_midas)
    #make column names first row
    filtered_AS_data_midas.columns = filtered_AS_data_midas.iloc[0]
    filtered_AS_data_midas = filtered_AS_data_midas[1:]
    print(filtered_AS_data_midas.columns)
    growing = filtered_AS_data_midas[filtered_AS_data_midas[f'SRT_{key}'] > 0]
    dying = filtered_AS_data_midas[filtered_AS_data_midas[f'SRT_{key}'] < 0]
...
# region to create barplot of functional groups present
...
# region to see Aerobic Heterotrophs over time
...
# region to see Nitrogen Reducers over time
...
# region to see Fermenters over time
...
filtered_AD_data_midas = filtered_AD_data_midas[['week', 'genus', 'SRT_AD']]
...
for i in range(len(filtered_AD_data_midas['genus'].unique())):
    ax2.plot(filtered_AD_data_midas[filtered_AD_data_midas['genus'] == filtered_AD_data_midas['genus'].unique()[i]]['week'],
             filtered_AD_data_midas[filtered_AD_data_midas['genus'] == filtered_AD_data_midas['genus'].unique()[i]]['SRT_AD'])
             #label=filtered_AS_data_midas['genus'].unique()[i])
ax2.set_xlabel('Time (weeks)')
ax2.set_ylabel('SRT')
ax2.set_title('SRT over time in AD system for Fermenters')
...
filtered_AD_data_midas = filtered_AD_data_midas[['week', 'genus', 'SRT_AD']]
#transpose data
filtered_AD_data_midas = filtered_AD_data_midas.pivot(index='week', columns='genus', values='SRT_AD')
#remove columns with missing values
filtered_AD_data_midas = filtered_AD_data_midas.dropna(axis=1)
filtered_AD_data_midas = filtered_AD_data_midas.to_numpy()
#save filtered data
np.save(f'{file_path}fermenters_over_time.npy', filtered_AD_data_midas)